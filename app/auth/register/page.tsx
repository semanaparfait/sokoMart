...
export default function RegisterPage() {
  ...
  const [form, setForm] = useState({
    name: '', email: '', password: '',
    role: (searchParams.get('role') as 'buyer' | 'seller') || 'buyer',
  });
        ...
        <div className="bg-white rounded-3xl shadow-xl shadow-emerald-100/60 p-8">
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            {[
              ...
              <button key={role} type="button" onClick={() => setForm({ ...form, role: role as 'buyer' | 'seller' })}
                className={`flex flex-col items-center gap-2 p-4 rounded-2xl border-2 transition-all ${form.role === role ? 'border-emerald-500 bg-emerald-50' : 'border-gray-200 hover:border-gray-300'}`}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {form.role === 'seller' && (
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-700">
                <strong>Note:</strong> As a seller, your products will be reviewed by our admin team before going live on the marketplace.
              </div>
            )}
